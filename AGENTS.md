...
2. **Preserve evidence.** Capture every source you rely on with `sw fetch URL SOURCE`, where SOURCE is the slug of its page in `wiki/sources/`. A source is one URL with one current capture. `sw fetch` stores the original and a Markdown reading copy in the private R2 store under the SHA-256 of the original, prints the capture and the path of the local Markdown copy, indexes it for `sw search`, and creates the source page with generated front matter or records the capture on it. A URL that is already captured is not fetched again: `sw fetch` returns its current capture. Add `--recapture` only when that capture is stale or broken; the new capture replaces it, and you recheck the page's Key points against it. Read the Markdown copy instead of fetching the page again. For a copy you saved from a browser, such as a paywalled page, add `--file FILE`; `sw fetch --hash SHA` restores any stored capture. Never link to stored originals. A repository is its README, fetched by raw URL at a fixed commit; fetch other files of it without SOURCE and cite them on the repository's page by locator. Read sources yourself; use a subagent only for discovery across many sources. `sw fetch` and `sw search` get their credentials through secretspec from the git-ignored `.env` of the primary worktree; if it reports missing secrets, ask the user to add them there.
