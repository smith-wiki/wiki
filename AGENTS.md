...
2. **Preserve evidence.** Capture every source you rely on with `sw fetch URL SOURCE`, where SOURCE is the slug of its page in `wiki/sources/`. It stores the original and a Markdown reading copy in the private R2 store under the SHA-256 of the original, prints the capture and the path of the local Markdown copy, and creates the source page with generated front matter or adds the capture to an existing page. Read that Markdown instead of fetching the page again. For a copy you saved from a browser, such as a paywalled page, add `--file FILE`; `sw fetch --hash SHA` restores any stored capture. Never link to stored originals. For a repository, fetch files by their raw URL at a fixed commit into the repository's page. Read sources yourself; use a subagent only for discovery across many sources. `sw fetch` gets its credentials through secretspec from the git-ignored `.env` of the primary worktree; if it reports missing secrets, ask the user to add them there.
