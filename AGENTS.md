...
- `wiki/sources/`: one page per independently citable original source. State the original URL, author or responsible organization, publication date or version when given, retrieval time (UTC), raw path, and SHA-256 digest. List the statements the wiki relies on, each with a locator: a heading or exact text for a webpage, a page or section for a paper, a commit and path for a repository. Record caveats such as promotional framing, staleness, or self-reported results when they matter.
