...
- End factual claims or paragraphs with citations to preserved source notes. Use route-relative Markdown links, for example `[S1](../sources/source-slug/)` from a research route; source notes must identify the original URL, retrieval time, content digest, and local raw evidence path.
