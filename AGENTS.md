...
1. **Orient.** Run `sw` and `sw search` with the turn's question, and read the existing pages the turn touches. `sw search` finds passages in every current capture, including sources fetched earlier that no page uses yet, and in the wiki; `--in sources` or `--in wiki` narrows it. A hit is a pointer: read the lines it names in the local Markdown copy before relying on it.
2. **Preserve evidence.** Capture every source you rely on with `sw fetch URL SOURCE`, where SOURCE is the slug of its page in `wiki/sources/`. A source is one URL with one current capture. `sw fetch` stores the original and a Markdown reading copy in the private R2 store under the SHA-256 of the original, prints the capture and the path of the local Markdown copy, indexes it for `sw search`, and creates the source page with generated front matter or records the capture on it. A URL that is already captured is not fetched again: `sw fetch` returns its current capture. Add `--recapture` only when that capture is stale or broken; the new capture replaces it, and you recheck the page's Key points against it. Read the Markdown copy instead of fetching the page again. For a copy you saved from a browser, such as a paywalled page, add `--file FILE`; `sw fetch --hash SHA` restores any stored capture. Never link to stored originals. A PDF's or arXiv paper's Markdown copy keeps its figures as images under `assets/` beside it, each followed by a quoted figure description that a vision model wrote so search can find it. The description is marked as generated and is never evidence: open the image before relying on a figure, and cite the figure, not the description. For an arXiv paper, pass any arxiv.org link: the source becomes the paper's versioned PDF, `https://arxiv.org/pdf/<id>v<n>`, and its Markdown copy comes from arXiv's HTML rendering, formulas included. A repository is its README, fetched by raw URL at a fixed commit; fetch other files of it without SOURCE and cite them on the repository's page by locator. Read sources yourself; use a subagent only for discovery across many sources. `sw fetch` and `sw search` get their credentials through secretspec from the git-ignored `.env` of the primary worktree; if it reports missing secrets, ask the user to add them there.
...
Write Markdown in your own words and quote only when the wording matters. Cite with readable author-date links to source pages, using the source's `author` and the year of `published`, or `n.d.` when it has none, such as `([Lamport, 2025](../../sources/leslie-lamport-tla-homepage/))`. Attribute strong claims to their source. Prefix interpretation with **Inference:**, open gaps with **Uncertainty:**, and conflicting evidence with **Contradiction:**.
