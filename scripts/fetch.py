...
from convert import (NETWORK_ERRORS, VISION_MODEL, Assets, ConvertError, Paper, arxiv_markdown, arxiv_paper, describe_figures, get,
                     pdf_markdown, send)
...
def firecrawl_scrape(url: str, formats: list[str]) -> dict:
    ...
    try:
        payload = json.loads(send(request, 180)[0])
...
def capture_url(url: str, file: Path | None, paper: Paper | None) -> tuple[Capture, bytes, str | None, Assets, str]:
    ...
    markdown, note = with_descriptions(markdown, assets)
    method += note
...
def with_descriptions(markdown: str | None, assets: Assets) -> tuple[str | None, str]:
    """MARKDOWN with generated figure descriptions, and a note for the method line.

    A failing vision model leaves the figures undescribed instead of failing the capture;
    `--reparse` describes them later.
    """
    if not markdown or not assets:
        return markdown, ""
    try:
        described, count = describe_figures(markdown, assets)
    except (*NETWORK_ERRORS, KeyError, IndexError) as error:
        from search import Health, error_class  # search imports this module
        Health(primary_root()).report("vision", error_class(error))
        return markdown, f"; figures not described ({error_class(error)}), run sw fetch --hash SHA --reparse later"
    return described, f"; {count} of {len(assets)} figures described by {VISION_MODEL}"


def reparse(store: Store, sha: str) -> tuple[Capture, Path, str]:
    """Rebuild a stored capture's Markdown copy, figures, and figure descriptions with the current converters."""
    ...
    markdown, note = with_descriptions(markdown, assets)
    return capture, store.replace_copy(capture, markdown, assets), "Markdown copy rebuilt" + note
...
def print_capture(capture: Capture, markdown: Path | None, stored: str, method: str | None = None) -> None:
    ...
    if markdown:
        ...
        described = text.count("> Figure description (generated by")
        print(f"markdown: {markdown} ({len(text.splitlines())} lines, {len(text.split())} words, "
              f"{figures} figures, {described} described)")
    else:
...
def main() -> int:
    ...
    try:
        if args.sha:
            ...
            if args.reparse:
                capture, markdown, method = reparse(store, args.sha)
